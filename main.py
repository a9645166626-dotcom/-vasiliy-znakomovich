...
async def my_profile(message: Message):
    ...
    if user[8] != "done":
        ...
        await message.answer(
            "🦊 <b>Анкета ещё не готова.</b>\n\n"
            "Сначала закончи регистрацию.",
            parse_mode="HTML"
        )
...
async def my_matches(message: Message):
    ...
    rows = con.execute(
        """
        SELECT DISTINCT u.name, u.age, u.city
        FROM users u
        JOIN likes l1
            ON l1.to_id = u.id
        JOIN likes l2
...
async def settings(message: Message):
    ...
    await message.answer(
        ...
        "━━━━━━━━━━━━━━\n\n"
        "🔄 Пересоздать анкету\n"
        "🛡️ Здесь можно будет настроить приватность\n"
        "🔎 Здесь появятся дополнительные настройки поиска.",
        parse_mode="HTML",
...
async def support(message: Message):
    ...
    await message.answer(
        ...
        "напиши сообщение следующим сообщением.\n\n"
...
async def registration(message: Message):
    ...
    # ВАЖНО:
    # кнопки меню уже имеют свои обработчики.
    # Здесь их НЕ трогаем.
