...
async def start(message: Message):
    ...
    await message.answer(
        f"🦊 <b>Продолжим создание анкеты.</b>\n\n"
        f"{state_text.get(current_state, 'Продолжим.')}",
        parse_mode="HTML"
    )
...
async def registration(message: Message):
    ...
    if state == "name":
        ...
        if len(text) < 2:
            ...
            await message.answer(
                "🦊 Напиши настоящее имя или хотя бы 2 символа."
            )
...
async def choose_looking(callback: CallbackQuery):
    ...
    if age < 18:
        ...
        text = (
            ...
            "показываются только анкеты 13–17 лет."
        )
        ...
        text = (
            ...
            "━━━━━━━━━━━━━━\n\n"
            "📍 <b>В каком городе ты живёшь?</b>\n\n"
            "Для пользователей 18+ показываются\n"
            "только анкеты 18+."
        )
...
async def show_profile(message, user_id):
    ...
    # -----------------------------------------------------
    # НЕ ПОКАЗЫВАТЬ УЖЕ ОБРАБОТАННЫЕ АНКЕТЫ
    # -----------------------------------------------------
    ...
    if not person:
        ...
        await message.answer(
            ...
            "Мы показываем только людей из твоего города\n"
...
async def like(callback: CallbackQuery):
    ...
    if mutual and target:
        ...
        target_name = html.escape(str(target[0]))
        ...
        if target_username:
            ...
            await callback.message.answer(
                "💕 <b>МЭТЧ!</b>\n"
                "━━━━━━━━━━━━━━\n\n"
                f"Вы понравились друг другу с <b>{target_name}</b>! 🧡\n\n"
                "Теперь вы можете открыть профиль друг друга\n"
...
async def settings(message: Message):
    ...
    await message.answer(
        ...
        "Фильтры поиска сейчас работают автоматически:\n"
        "📍 город\n"
...
async def support(message: Message):
    ...
    await message.answer(
        "🆘 <b>ПОДДЕРЖКА ЛИСИ</b>\n"
        "━━━━━━━━━━━━━━\n\n"
        "Если у тебя возникла проблема или есть предложение,\n"
...
# =========================================================
# НЕПРАВИЛЬНЫЙ ТЕКСТ ВО ВРЕМЯ ОЖИДАНИЯ ФОТО
# =========================================================
